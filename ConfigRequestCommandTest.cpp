...
TEST_F(ConfigRequestCommandTest, BaseConfTwoSetOfValues) {
   ...
   MockConfigRequestCommand::ManyConfDefaults defaults;
   auto dpiThreads = std::make_tuple("dpiThreads", MockConfigRequestCommand::Ranges{"2", "7", "1"});
   auto qosmosDebugEnabled = std::make_tuple("qosmosDebugModeEnabled", MockConfigRequestCommand::Ranges{"false", "", ""});
   defaults.push_back(dpiThreads);
   ...
   EXPECT_EQ(readQosmosDebug.maxu64(), "");
   EXPECT_EQ(readQosmosDebug.minu64(), "");    
#endif
}
