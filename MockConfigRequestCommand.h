...
struct MockConfigRequestCommand : public ConfigRequestCommand {   
   ...
   typedef std::tuple<std::string, Ranges> OneConfDefault;
      ...
      if (request.has_type() && size > 0) { 
         auto allDefaultsForType = HelperGetConfDefaults(request.type());
            for (int index = 0; index < size; ++index) {
               ...
               if (HelperLoadConfDefault(setting, getDefaultsForMe, allDefaultsForType)) {
                  protoMsg::ConfigDefaults_DefaultValues values;
                  values.set_type(request.type());
                  values.set_configname(std::get<0>(setting));
                  values.set_defaultu64(std::get<1>(setting).Default);
                  values.set_maxu64(std::get<1>(setting).Max);
                  values.set_minu64(std::get<1>(setting).Min);
