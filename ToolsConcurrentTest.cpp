...
TEST(TestOfConcurrent, MoveOnCopy__CompilerCheckForStringCallWithObjectArg) {
    ...
    auto futureHello1 = hello.call(&GreetingWithUnique::talkBack, MoveOnCopy<UniqueGreeting>(std::move(arg)));
    EXPECT_EQ("Hello World", futureHello1.get());

    auto futureHello2 = hello.call(&GreetingWithUnique::talkBack, MoveOnCopy<UniqueGreeting>(std::move(arg)));
