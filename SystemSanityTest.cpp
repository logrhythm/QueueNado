...
TEST_F(SystemSanityTest, GlibCallsUsingFullPath__ShouldNotAffectOtherDirectories) {
   ...
   auto changeChmod = [&maxFiles](const std::string& path, mode_t mode, std::atomic<bool>* removeHasFinished) ->bool {
      bool success = true;
      while (false == removeHasFinished->load() && !zctx_interrupted) {
         for (size_t index = 0; index < maxFiles; ++index) {
   ...
   auto removeFiles = [&maxFiles](const std::string& path, std::atomic<bool>* hasFinished) ->bool {
      bool success = true;
      for (size_t index = 0; !zctx_interrupted && (index < maxFiles); ++index) {
         std::string filepath = {path + "/" + std::to_string(index)};
   ...
   std::atomic<bool> hasRemoveFinished{false};
   
   finished.push_back(std::async(std::launch::async, changeChmod, dirChange1, 777, &hasRemoveFinished));
   finished.push_back(std::async(std::launch::async, changeChmod, dirChange2, 555,  &hasRemoveFinished));
   finished.push_back(std::async(std::launch::async, removeFiles, dirRemove,  &hasRemoveFinished));
